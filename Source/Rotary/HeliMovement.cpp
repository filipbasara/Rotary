...
#define Self URotary_HeliMovementComponent


Self::URotary_HeliMovementComponent() : Super()
{
...
auto Self::ComputeTorque(FVector const& angularVelocity, float mass) const -> FVector
{
	...
	auto roll = FoRotaryrd() * -m_Input.Roll * CyclicSensitivity;
...
auto Self::FoRotaryrd() const -> FVector
{
	auto* pawn = GetPawn();
...
auto Self::GetHeadingDegrees() const -> float
{
	auto direction = FVector
		::VectorPlaneProject(FoRotaryrd(), FVector::UpVector)
		.GetSafeNormal();
