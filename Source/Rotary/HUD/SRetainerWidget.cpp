...
#define Self SRotary_RetainerWidget

DECLARE_CYCLE_STAT(
	TEXT("Rotary Retainer Widget Tick"),
	STAT_Rotary_RetainerWidgetTick,
	STATGROUP_Slate);
DECLARE_CYCLE_STAT(
	TEXT("Rotary Retainer Widget Paint"),
	STAT_Rotary_RetainerWidgetPaint,
	STATGROUP_Slate);

#if !UE_BUILD_SHIPPING
FRotary_OnRetainedModeChanged Self::s_OnRetainedModeChangedDelegate {};
#endif
...
auto Rotary_EnableRetainedRendering = FAutoConsoleVariableRef{
	TEXT("Slate.RotaryEnableRetainedRendering"),
	g_EnableRetainedRendering,
	TEXT("Whether to attempt to render things in SRotary_RenderEffectsWidgets to "
		"render targets first"),
};

bool g_SlateEnableRenderWithLocalTransform = true;
auto CVarGlateRotary_EnableRenderWithLocalTransform = FAutoConsoleVariableRef{
	TEXT("Slate.RotaryEnableRetainedRenderingWithLocalTransform"),
...
public:
	FRotary_RenderResources() = default;
	
	~FRotary_RenderResources()
	{
	...
	{
		return TEXT("FRotary_RenderResources");
	}
};


// SRotary_RetainerWidget =========================================================
...
void Self::Construct(FArguments const& args)
{
	...
	if (FSlateApplication::IsInitialized()) {
		...
		if (!s_initialized) {
			...
			Rotary_EnableRetainedRendering->SetOnChangedCallback(
	...
	-> EPaintRetainedContentResult
{
	...
	if (FMath::Max(rtWidth, rtHeight) > GetMax2DTextureDimension()) {
		...
		UE_LOG(LogTemp, Error,
			TEXT("The requested size for SRotary_RetainerWidget is too large! "
				"Width: %i; Height: %i;"),
			rtWidth, rtHeight);
	...
	if (rtWidth == 0 || rtHeight == 0) {
		// TODO: Replace with a proper logger category
		UE_LOG(LogTemp, Error,
			TEXT("The requested size of SRotary_RetainerWidget has a zero dimension! "
				"Width: %i; Height: %i;"),
			rtWidth, rtHeight);
	...
	const -> int32
{
	...
	SCOPE_CYCLE_COUNTER(STAT_Rotary_RetainerWidgetPaint);
